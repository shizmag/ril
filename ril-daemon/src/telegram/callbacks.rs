...
pub async fn handle_callback_query(
    ...
    if data == "hub" {
        ...
        let id: i64 = data.split(':').nth(1).and_then(|s| s.parse().ok()).unwrap_or(0);
        ...
        let id: i64 = data.split(':').nth(1).and_then(|s| s.parse().ok()).unwrap_or(0);
        bot.edit_message_text(msg.chat.id, msg.id, "⭐ <b>Пожалуйста, выберите оценку для материала:</b>")
        ...
        if parts.len() == 3 {
            ...
            if rating.is_some() {
                state.set_pending_state(user.id, PendingState::WaitingForComment { article_id: id }).await;
        ...
        let id: i64 = data.split(':').nth(1).and_then(|s| s.parse().ok()).unwrap_or(0);
        state.set_pending_state(user.id, PendingState::WaitingForComment { article_id: id }).await;
        bot.edit_message_text(msg.chat.id, msg.id, "💬 <b>Пожалуйста, отправьте ваш комментарий следующим сообщением:</b>")
            .reply_markup(keyboards::back_to_article_keyboard(id))
            .parse_mode(teloxide::types::ParseMode::Html)
            .await?;
        ...
        let id: i64 = data.split(':').nth(1).and_then(|s| s.parse().ok()).unwrap_or(0);
        state.set_pending_state(user.id, PendingState::WaitingForTag { article_id: id }).await;
        bot.edit_message_text(msg.chat.id, msg.id, "🏷 <b>Пожалуйста, отправьте название тега (или несколько через запятую):</b>")
            .reply_markup(keyboards::back_to_article_keyboard(id))
            .parse_mode(teloxide::types::ParseMode::Html)
            .await?;
        ...
        let page: i64 = data.split(':').nth(1).and_then(|s| s.parse().ok()).unwrap_or(0);
        ...
        bot.edit_message_text(msg.chat.id, msg.id, "✅ <b>Библиотека полностью очищена.</b>")
            .reply_markup(keyboards::back_to_hub_keyboard())
            .parse_mode(teloxide::types::ParseMode::Html)
            .await?;
    } else if data == "search" {
        ...
        state.set_pending_state(user.id, PendingState::WaitingForSearchQuery).await;
        bot.edit_message_text(msg.chat.id, msg.id, "🔎 <b>Введите поисковый запрос (текст или ключевые слова):</b>")
            .reply_markup(keyboards::back_to_article_keyboard(0)) // fallback to cancel
            .parse_mode(teloxide::types::ParseMode::Html)
            .await?;
    } else if data == "sf_domain" {
        state.set_pending_state(user.id, PendingState::WaitingForFilterDomain).await;
        bot.edit_message_text(msg.chat.id, msg.id, "🌐 <b>Введите домен сайта для фильтрации (например, habr.com или medium.com):</b>")
        ...
        state.update_search_session(user.id, |s| {
            s.tag = if tag == "none" { None } else { Some(tag.to_string()) };
        }).await;
        show_search_menu(bot, msg, user.id, state).await?;
    } else if data == "sf_status" {
        bot.edit_message_text(msg.chat.id, msg.id, "📝 <b>Выберите статус прочтения для фильтрации:</b>")
        ...
        bot.edit_message_text(msg.chat.id, msg.id, "⭐ <b>Выберите оценку для фильтрации:</b>")
        ...
        bot.edit_message_text(msg.chat.id, msg.id, "📅 <b>Выберите интервал добавления:</b>")
...
async fn show_list(
    ...
    let status_filter = if status == "all" { None } else { Some(status.to_string()) };
    
    let paginated = state
        .bridge
        .search_articles_advanced(None, status_filter, None, None, None, false, false, None, limit, offset)
        .await
        .tg_err()?;
        
    let total_pages = (paginated.total_count as f64 / limit as f64).ceil() as i64;
    
    let prev_cb = if page > 0 { Some(format!("list:{}:{}", status, page - 1)) } else { None };
    let next_cb = if page + 1 < total_pages { Some(format!("list:{}:{}", status, page + 1)) } else { None };
    ...
    bot.edit_message_text(msg.chat.id, msg.id, text)
        .reply_markup(keyboards::pagination_keyboard(prev_cb, next_cb, "hub"))
        .parse_mode(teloxide::types::ParseMode::Html)
        .await?;
    Ok(())
}
...
async fn show_comment_menu(
    ...
    let content = state.bridge.get_article_content(id).await.tg_err()?;
    let current_comm = content.article.comment.as_deref().unwrap_or("<i>комментарий отсутствует</i>");
    let text = format!(
...
async fn show_article_tags(
    ...
    let content = state.bridge.get_article_content(id).await.tg_err()?;
    let mut rows = vec![];
    rows.push(vec![InlineKeyboardButton::callback("➕ Добавить тег", format!("tag_add:{}", id))]);
    ...
    rows.push(vec![InlineKeyboardButton::callback("🔙 Назад", format!("art:{}", id))]);
    let markup = InlineKeyboardMarkup::new(rows);
    
    let tags_text = if content.article.tags.is_empty() {
        "нет тегов".to_string()
    } else {
        content.article.tags.iter().map(|t| format!("#{}", t)).collect::<Vec<_>>().join(", ")
    };
...
async fn show_tags_list(
    ...
    bot.edit_message_text(msg.chat.id, msg.id, text)
        .reply_markup(keyboards::tags_list_keyboard(&paged_tags, page, total_pages))
        .parse_mode(teloxide::types::ParseMode::Html)
        .await?;
    Ok(())
}
...
async fn show_articles_by_tag(
    ...
    let paginated = state
        .bridge
        .search_articles_advanced(None, None, Some(tag.to_string()), None, None, false, false, None, limit, offset)
        .await
        .tg_err()?;
    let total_pages = (paginated.total_count as f64 / limit as f64).ceil() as i64;
    
    let prev_cb = if page > 0 { Some(format!("stag:{}:{}", tag, page - 1)) } else { None };
    let next_cb = if page + 1 < total_pages { Some(format!("stag:{}:{}", tag, page + 1)) } else { None };
    
    let text = views::render_articles_list(&paginated.articles, &format!("Тег: #{}", tag), page, total_pages);
    bot.edit_message_text(msg.chat.id, msg.id, text)
        .reply_markup(keyboards::pagination_keyboard(prev_cb, next_cb, "tags_list:0"))
        .parse_mode(teloxide::types::ParseMode::Html)
        .await?;
    Ok(())
}
...
async fn show_articles_by_rating(
    ...
    let paginated = state
        .bridge
        .search_articles_advanced(None, None, None, rating_opt, None, false, no_rating, None, limit, offset)
        .await
        .tg_err()?;
    let total_pages = (paginated.total_count as f64 / limit as f64).ceil() as i64;
    
    let prev_cb = if page > 0 { Some(format!("srate:{}:{}", rating, page - 1)) } else { None };
    let next_cb = if page + 1 < total_pages { Some(format!("srate:{}:{}", rating, page + 1)) } else { None };
    
    let title = if rating == 0 { "Материалы без оценки".to_string() } else { format!("Оценка: {} ⭐", rating) };
    let text = views::render_articles_list(&paginated.articles, &title, page, total_pages);
    bot.edit_message_text(msg.chat.id, msg.id, text)
        .reply_markup(keyboards::pagination_keyboard(prev_cb, next_cb, "ratings_list"))
        .parse_mode(teloxide::types::ParseMode::Html)
        .await?;
    Ok(())
}
...
async fn show_search_tag_select(
    ...
    rows.push(vec![InlineKeyboardButton::callback("❌ Сбросить тег", "sft_select:none")]);
    rows.push(vec![InlineKeyboardButton::callback("🔙 Назад к поиску", "search")]);
    let markup = InlineKeyboardMarkup::new(rows);
...
async fn run_search(
    ...
    let total_pages = (paginated.total_count as f64 / limit as f64).ceil() as i64;
    let prev_cb = if page > 0 { Some(format!("sf_run:{}", page - 1)) } else { None };
    let next_cb = if page + 1 < total_pages { Some(format!("sf_run:{}", page + 1)) } else { None };
    
    let text = views::render_articles_list(&paginated.articles, "Результаты поиска", page, total_pages);
    bot.edit_message_text(msg.chat.id, msg.id, text)
        .reply_markup(keyboards::pagination_keyboard(prev_cb, next_cb, "search"))
        .parse_mode(teloxide::types::ParseMode::Html)
        .await?;
    Ok(())
}
...
async fn show_ratings_list(bot: Bot, msg: &Message) -> ResponseResult<()> {
    ...
    rows.push(vec![InlineKeyboardButton::callback("❌ Без оценки", "srate:0:0")]);
