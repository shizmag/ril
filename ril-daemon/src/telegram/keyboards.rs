...
pub fn hub_keyboard() -> InlineKeyboardMarkup {
    InlineKeyboardMarkup::new(vec![
        vec![
            InlineKeyboardButton::callback("📚 Все материалы", "list:all:0"),
        ],
        vec![
...
pub fn article_card_keyboard(art_id: i64, status: &str, url: &str) -> InlineKeyboardMarkup {
    ...
    InlineKeyboardMarkup::new(vec![
        vec![
            InlineKeyboardButton::url("📄 Открыть", url.parse().unwrap_or_else(|_| "https://google.com".parse().unwrap())),
            status_btn,
        ],
        vec![
...
pub fn rating_keyboard(art_id: i64) -> InlineKeyboardMarkup {
    InlineKeyboardMarkup::new(vec![
        ...
        vec![
            InlineKeyboardButton::callback("❌ Сбросить оценку", format!("rate_set:{}:0", art_id)),
        ],
        vec![
            InlineKeyboardButton::callback("🔙 Назад", format!("art:{}", art_id)),
        ]
    ])
}
...
pub fn pagination_keyboard(prev_cb: Option<String>, next_cb: Option<String>, home_cb: &str) -> InlineKeyboardMarkup {
    let mut row = vec![];
...
pub fn search_menu_keyboard(session: &SearchSession) -> InlineKeyboardMarkup {
    ...
    let status_label = match &session.status {
        Some(s) => if s == "read" { "✅ Только прочитанные" } else { "📖 Только непрочитанные" },
        None => "📝 Любой статус"
    };
    ...
    let date_label = match &session.date_added {
        Some(d) => if d == "today" { "📅 За сегодня" } else if d == "week" { "📅 За неделю" } else { "📅 За месяц" },
        None => "📅 За всё время"
    };
    ...
    InlineKeyboardMarkup::new(vec![
        ...
        vec![
            InlineKeyboardButton::callback(date_label, "sf_date"),
        ],
        vec![
...
pub fn search_status_select_keyboard() -> InlineKeyboardMarkup {
    InlineKeyboardMarkup::new(vec![
        ...
        vec![
            InlineKeyboardButton::callback("📝 Любой статус", "sfs_any"),
        ],
        vec![
            InlineKeyboardButton::callback("🔙 Назад к поиску", "search"),
        ]
    ])
}
...
pub fn search_date_select_keyboard() -> InlineKeyboardMarkup {
    InlineKeyboardMarkup::new(vec![
        ...
        vec![
            InlineKeyboardButton::callback("📅 За всё время", "sfd_any"),
        ],
        vec![
            InlineKeyboardButton::callback("🔙 Назад к поиску", "search"),
        ]
...
pub fn stats_menu_keyboard() -> InlineKeyboardMarkup {
    InlineKeyboardMarkup::new(vec![
        ...
        vec![
            InlineKeyboardButton::callback("📅 Динамика", "stats:dynamics"),
        ],
        vec![
            InlineKeyboardButton::callback("🏠 В хаб", "hub"),
        ]
...
pub fn tags_list_keyboard(tags: &[TagStat], page: i64, total_pages: i64) -> InlineKeyboardMarkup {
    ...
    let mut pag_row = vec![];
    if page > 0 {
        pag_row.push(InlineKeyboardButton::callback("⬅️ Назад", format!("tags_list:{}", page - 1)));
    }
    if page + 1 < total_pages {
        pag_row.push(InlineKeyboardButton::callback("➡️ Далее", format!("tags_list:{}", page + 1)));
    }
    if !pag_row.is_empty() {
        rows.push(pag_row);
    }
    ...
    InlineKeyboardMarkup::new(vec![
        vec![
            InlineKeyboardButton::callback(
                if current_format == "markdown" { "✅ Markdown" } else { "Markdown" },
                "set_fmt:markdown"
            ),
            InlineKeyboardButton::callback(
                if current_format == "html" { "✅ HTML" } else { "HTML" },
                "set_fmt:html"
            ),
            InlineKeyboardButton::callback(
                if current_format == "epub" { "✅ EPUB" } else { "EPUB" },
                "set_fmt:epub"
            ),
        ],
        vec![
            InlineKeyboardButton::callback("⚠️ Сбросить библиотеку", "reset_lib_prompt"),
        ],
        vec![
            InlineKeyboardButton::callback("🏠 В хаб", "hub"),
        ]
...
pub fn back_to_article_keyboard(art_id: i64) -> InlineKeyboardMarkup {
    InlineKeyboardMarkup::new(vec![
        vec![InlineKeyboardButton::callback("🔙 Назад к материалу", format!("art:{}", art_id))]
    ])
}
